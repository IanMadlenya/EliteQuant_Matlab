...
function [output headers] = get(code, varargin)
    ...
    for i = 1:rowz
        ...
        if i == 1
            %DATE = csv{i+1}(1:10);
            DATE = {datenum(csv{i+1}(1:10),'yyyy-mm-dd')};
            %DATE = datestr(datenum(csv{i+1}(1:10),'yyyy-mm-dd'),'mm/dd/yyyy');
        else
             %DATE = char(DATE,csv{i+1}(1:10));
            DATE = [DATE; {datenum(csv{i+1}(1:10),'yyyy-mm-dd')}];
            %DATE = char(DATE,datestr(datenum(csv{i+1}(1:10),'yyyy-mm-dd'),'mm/dd/yyyy'));
        end
        data(i,:) = temp;
    end
    %DATE = cellstr(DATE);    % ElteQuant
    temp = size(type);
